...
class musicoController extends Controller
{
    ...
    public function perfilPublico($id) 
    {
        try { 
            ...
            $newId=User::where('spotify_id',$id)->value('id');
        } catch(QueryException $ex){ 
            return view('errors.404', ['mensaje' => 'No fue posible conectarse con la base de datos']);
        }
        ...
        return view ('musico.PublicoperfilMusico', ['usuario' => $usuario,'campsAct'=>$campsAct,'songsAct'=>$songsAct,'playlistsAct'=>$playlistsAct,'campsAnt'=>$campsAnt,'songsAnt'=>$songsAnt,'playlistsAnt'=>$playlistsAnt,'error'=>$error]);
    }
